...
estimating extinction factors in spherical crystallites is provided,
addressing limitations in scenarios with higher Bragg angles. The work utilises
modern computing capabilities and updated recipes are provided for easy
community adoption.
\end{synopsis}

\begin{abstract}
We present a technical correction and update of the widely used recipe by
Becker-Coppens (1974), for the estimation of extinction factors in a perfect
spherical crystallite. In the original publication, these extinction factors
were evaluated numerically from a complicated integral, and simplified
analytical approximations to these numerical evaluations constituted the
provided recipes. However, in case of significant level of extinction, the
original recipes for primary extinction suffers from numerical break-down in
...
revisited, and updated recipes are provided which can be used also for
back-scattering and strong extinction scenarios. In order to facilitate easy
adoption by the community, the new recipes are compared to the old, and two new
recipes are provided: a minimalistic update of the 4 free parameters in the old
recipes, and a more complicated and precise analytical form with 21 free
parameters. Both new recipes solve the issues of numerical break-down. For
consistency, secondary extinction recipes are updated as well.
\end{abstract}

\keywords{extinction; bragg diffraction; neutron scattering; X-ray diffraction}

\section{The Becker-Coppens recipe for primary extinction}
...
\reffig{ypbreakdownorigrecipe} shows the original motivation (bla. NCrystal\cite{ncrystalmain,ncrystalelastic}.):

\begin{figure}[ht] %
\labfig{ypbreakdownorigrecipe}%fixme
\begin{center}
\includegraphics[width=0.5\textwidth]{../data/ncrystal_bc_yp_breakdown_be.pdf} % NB use pdflatex for non-postscript
\end{center}
\caption{Bla. Beryllium.} % NB \protect\cite{...} is required in floating figures
\end{figure}

Define yp integral, x, theta, f, phi0, phipi.
...
\subsection{Definition of model precision}
...
\begin{align}
  y_M(\theta,x) = N_M\int_{0}^{\infty} f_M(\eta)\varphi\left(\theta,C_Mxf_M(\eta)\right)d\eta
  \labeqn{ypintegral}%fixme rename
\end{align}
...
Note that the definition of $x$ itself also depends on the model, but we will
...
Here the subscript $M$ is a placeholder for $P$ (primary), $G$ (secondary Gaussian), $L$
(secondary Lorentzian), or $F$ (secondary Fresnellian).

\subsection{Old recipe}
...
To produce equivalent of \refeqn{ypintegral} (BC.36), one must go back to BC.14,
...
Mention OCR(tesseract)+human checking.
...
\input{generated/recipes.tex}


\begin{figure}[ht] %
\labfig{cmprecipes:primary}%fixme
\begin{center}
\includegraphics[width=0.5\textwidth]{generated/cmprecipes_primary.pdf}
\end{center}
\caption{cmprecipes primary.}
\end{figure}

\begin{figure}[ht] %
\labfig{cmprecipes:primary:lux}%fixme
\begin{center}
\includegraphics[width=0.5\textwidth]{generated/cmprecipes_primary_lux.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.5\textwidth]{generated/cmprecipes_scndgauss.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.5\textwidth]{generated/cmprecipes_scndgauss_lux.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.5\textwidth]{generated/cmprecipes_scndlorentz.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.5\textwidth]{generated/cmprecipes_scndlorentz_lux.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.5\textwidth]{generated/cmprecipes_scndfresnel.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.5\textwidth]{generated/cmprecipes_scndfresnel_lux.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.5\textwidth]{generated/prec_classicrecipe_2d_primary.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.5\textwidth]{generated/prec_classicrecipe_2d_scndgauss.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.5\textwidth]{generated/prec_classicrecipe_2d_scndlorentz.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.5\textwidth]{generated/prec_classicrecipe_2d_scndfresnel.pdf}
\end{center}
\caption{precision 2d classic recipe (vs. new lux recipe). This is scndfresnel.}
\end{figure}


\subsection{Extrapolation to extremely high $x$ values}

Bla bla power law. Show scaling plot and approximate fit. Mention that there
will be no well determined precision of the provided recipes at these extreme
values, but that the solution with a simple power law is both simple and
trivially ensures well behaved numerical results not matter the size of x.
