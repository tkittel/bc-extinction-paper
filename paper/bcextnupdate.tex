...
\begin{table}[ht]
\caption{Bla bla FIXME: table location + use same precision def as in rest of paper?.} % NB \protect\cite{...} is required in floating tables
\smallskip
\tiny
\begin{center}
%%%% Begin code exported from pandas .to_latex() (NOTE: color statements needed
%%%% manual modification)
\input{../data/table1_updated.tex}
%%%% End of code exported from pandas .to_latex()
\end{center}
\end{table}

\begin{table}[ht]
\caption{Bla bla.} % NB \protect\cite{...} is required in floating tables
\smallskip
\tiny
\begin{center}
%%%% Begin code exported from pandas .to_latex() (NOTE: color statements needed
%%%% manual modification)
\input{../data/table1_diff.tex}
%%%% End of code exported from pandas .to_latex()
\end{center}
\end{table}
...
%%     if x < 0.1:
%%         # Taylor expansion:
%%         y0 = ( 1.0+x*(-0.94286+x*(0.82043+x*(-0.59332+x*(0.36445
%%                +x*-0.19429)))) )
%%     else:
%%         if x > 1e3:
%%             return bc2025_y_primary(1e3,sintheta)*(x*1e-3)**-0.5
%%         xp = bc2025_xprime(x)
%%         yp = yprime_primary_0(xp)
%%         y0 = bc2025_unprime_y( xp, yp )
%%     ss = sintheta**0.5
%%     xs = x*ss
%%     if xs < 0.1:
%%         # Taylor expansion:
%%         ydelta = ( xs*xs*(0.41022+xs*(-1.1866+xs*(2.3689+xs*-4.1771))) )
%%     else:
%%         xp = bc2025_xprime(xs)
%%         yp0 = yprime_primary_0(xp)
%%         yp90 = yprime_primary_90(xp)
%%         ydelta = (xp+1.0)**2 * (1.0-xp) * (yp90 - yp0)
%%     return y0 + sintheta*ss*ydelta
%%
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \Procedure{YP}{$x, \sin\theta$}
            \If{$x < 0.1$}
                \State $y_0 \gets (1+x\cdot(-0.94286+x\cdot(0.82043+x\cdot(-0.59332+x\cdot(0.36445 -x\cdot0.19429)))))$
            \Else
                \If{$x>1000$}
                ...
                \State $y_0'\gets(0.0182+x'\cdot(-0.1976+x'\cdot(0.3984+x'\cdot(0.2541+x'\cdot(-0.6802$\\
                \hspace{5em} ${\hookrightarrow}+x'\cdot(-0.2808+x'\cdot(0.8987+x'\cdot(0.007438+x'\cdot(-0.6028+x'\cdot0.248)))))))))$
                \State $y_0 \gets y_0'\cdot(1+x')^2\cdot(1-x')-(0.25{\cdot}x'\cdot(3-x'{\cdot}x')-0.5)$
                \EndIf
            \State $s \gets \sqrt{\sin\theta}$
            \State $u \gets s{\cdot}x$
            \If{$u < 0.1$}
                \State $y_\Delta \gets u{\cdot}u{\cdot}(0.41022-u{\cdot}(1.1866-u{\cdot}(2.3689-u{\cdot}4.1771)))$
            \Else
                \State $u_s \gets \sqrt{u} $
                \State $u' \gets (u_s-1)/(u_s+1) $
                \State $y_\Delta \gets \ldots u'...$
            \EndIf
            \State \textbf{return} $y_0+y_\Delta{\cdot}s\cdot\sin\theta$
        \EndProcedure
    \end{algorithmic}
\end{algorithm}
