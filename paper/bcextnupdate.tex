...
when the work was carried out moe than half a century ago, certainly
understandable.

\begin{align}
  y_{M}(\theta,x)=\frac{1}{\sqrt{1+t_Mx+\frac{A_M(\theta)}{1+x{\cdot}B_M(\theta)}x^2}}
  \labeqn{yfitfctorig}
\end{align}
...
\begin{align}
  ...
  \begin{cases}
    ...
    B_L(\theta) = -0.45\cos(2\theta)\quad&\text{when }\cos(2\theta) {<} 0
...
down in some cases. A few issues are even readily apparent just from inspection
of the formulae. Firstly and most importantly, it is possible for the last term
inside the square root in \refeqn{yfitfctorig} to become negative (e.g. at large
$\theta$ values this clearly happens for $M=P$), meaning that the argument of
the square root will also become negative for sufficiently large $x$ values,
causing the formula to break down completely. Another issue which is also
readily apparent is the behaviour at small $x$ values for $M=P$. The first order
Taylor expansion of \refeqn{yfitfctorig} is $1-t_M/2$ which can be compared with
the first order terms in \refeqnrange{ytaylor:P:first}{ytaylor:F:last}. Doing so
would predict $t_M$-values of $t_L=t_F=2$, $t_G=\sqrt{9/2}\approx2.12$, and
$t_P=66/35\approx1.89$. Thus, the $t_P$ values in \refeqn{yfitfctlowxconstants}
are incorrect, and it is easy to see that this gives a limiting precision
(cf.~\refeqn{yprecisiondef}) at small $x$-values of $2/33\approx6.1\%$.  Yet
another readily apparent issue is the incontinuity at $\cos(2\theta)=0$ in
\refeqn{yfitfct:BL}: the coefficients at first glance looks like they were
chosen to avoid this, since $0.15-2*0.75=0$, but the factor of $0.75$ actually
enters in quadrature.
...
the upper limit of applicability lying from around $x=1$ to $x=10$, with the
exact value depending naturally on the desired precision, on the model and in
particular also on $\theta$ values. In general, there is also a tendency for the
recipes to work more poorly for more backwards scattering.

NOTE: Table 2 of orig paper shows wiggles in sinth=0.9 column!
...
Make a point about it being >50 years ago, so maybe that is why they stopped at
sinth=0.9 and the focus being on forward values and cheap computation cost.
