...
scattering event will alternate the neutron's path between the diffracted
...
were not possible. The extinction factor, $y$, is defined as the ratio
...
model, but in general it will be proportional to $|Fa\lambda/V|^2$ where $F$ is
...
scattered, and $a$ some relevant length scale representing the path length of
...
Zachariasen \citeyear{zachariasen1967} improved upon by other authors like
Cooper-Rouse \citeyear{cooper1970extinction}, Becker-Coppens \citeyear{BC1974},
and Sabine \citeyear{sabine2006flow}. Of these, the Becker-Coppens model is
arguably the more highly referenced and utilised work\todo{Shuqi: and the model
  was confirmed in the authors’ prior work on extinction effects in
  CITE}, so it was naturally considered by the authors
when working on implementing extinction models in \texttt{NCrystal}
...
on the assumption of spherical geometries for the crystallites or particles. The
...
of transcription errors in the tables of the original work.  (FIXME: verify once
...
standalone functions are also made available at \cite{bc2025materials}.
...
recipes provide the guaranteed error bounds of respectively $10^{-3}$ and
...
detailed and impressive in the subjective opinion of the authors. In the near
future, these updated recipes will be adopted into \texttt{NCrystal}, and it is
the hope and recommendation of the authors that other refinement or modelling
codes supporting Becker-Coppens extinction will also be able to benefit from the
provided updated recipes.
...
\begin{align}
  ...
  \quad\text{and}\quad
  f_P(\eta) \ge f_P^-(\eta) \equiv \frac{\eta^2-\eta}{\eta^4}
\end{align}
The function $f^-(\eta)$ is only strictly positive when $\eta>1$, but for
simplicity it can be required that $a>2$.  Since $\varphi(\theta,s)$ and
...
\begin{align}
  ...
  \labeqn{tplusminus}
\end{align}
Here $k{\equiv}x/a$ and the requirement on $a$ has been tightened to
...
coefficients is always within a factor of $2.50$ when compared to the previous
order. Thus, by further restriction to $a>10$, it is possible to conservatively
limit the maximal contributions of the terms of order $1/a^4$ or higher in
\refeqn{tplusminus} to be at most $10/a^4$. With that, one can then use
$(T^+(\theta,x,a)+T^-(\theta,x,a))/2$ as an estimate of the tail integral value,
while $(T^+(\theta,x,a)-T^-(\theta,x,a))/2$ will be a conservative upper bound
of the error on the estimate. In other words, for $a>\max(10,x)$:
\begin{align}
\int_{a}^{\infty} f_P(\eta)\varphi\left(\theta,xf_P(\eta)\right)\deta \approx
...
\refeqn{tailintegralpenultimate}. Thus, by a slight additional pessimisation of
