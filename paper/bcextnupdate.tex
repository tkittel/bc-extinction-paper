...
\author[b]{Shuqi T. G. Xu\IUCrEmaillink{shuqi.xu@ess.eu}\IUCrOrcidlink{0000-0002-9615-7533}}%
\author[b]{J. I. M\'arquez Dami\'an\IUCrEmaillink{jose.marquez@ess.eu}\IUCrOrcidlink{0000-0002-9611-914X}}%
\affil[a]{ESS DMSC, European Spallation Source ERIC, Lyngby, Denmark}
...
factors in spherical crystallites is provided, addressing limitations in
scenarios with higher Bragg angles or stronger extinction effects, as well as
...
perfect spherical domains or particles, is presented. In the original work,
...
break-down in case of strong primary extinction effects, especially in backwards
...
intensity of a diffracted flux of particles is reduced or diminished due to
interference effects. The exact nature of these interference effects might be
complicated, and can in particular for {{\em} primary} extinction include higher
order interference between the incoming and scattered waves. For the case of
{{\em} secondary} extinction effects, the relevant interference can in general
be approximated at the particle transport level as an effect of the particle
undergoing multiple Bragg scattering's on the same reflection plane. Each
scattering event will alternate the neutron's orientation between the diffracted
and undiffracted directions, with the net effect being that the diffracted flux
is diminished compared to what one would expect if multiple scatterings
were not possible. The extinction factor, $y$, is defined as the difference
between the diffracted flux predicted by the kinematical model ($I_\text{kin}$)
with no multiple scattering or other dynamic effects, and the diffracted flux
which is actually observed once these effects are accounted for
($I_\text{obs}$):
\begin{align}
...
as a parameter $x$ which capture the general strength of the extinction
...
scattered, and $l$ some relevant length scale representing the path length of
...
of stronger extinction effects, the model appears to suffer from an obvious
...
that work. It is the aim of the present work to provide updated recipes which
...
monotonically decreasing functions of $s$ for $s\ge0$, attaining values $1$ at
$s=0$ and $0$ at $s=\infty$.
\begin{figure}[tp] %
\begin{center}
...
the shape of $f_M(\eta)$ is essentially given by the mosaic distribution of
coherent crystal domains within material particles, while for primary extinction
it is due to the intrinsic response of a crystalline domain which contains a
perfect and unbroken crystal structure, but is of finite size. Additionally, the
exact final form of the $f_M(\eta)$ functions is based on the assumption of
spherical geometries for the crystallites or particles. The exact form of these
functions, plotted in \reffig{fofeta}, is as follows:
\begin{figure}[tp] %
\begin{center}
...
reproduce its values with sufficient precision. As such recipes would
...
complexity in the updated recipes is therefore allowed.
...
$y_\text{ref}$ to itself be available in a precision significantly higher than
the precision on $y$.
...
present work are provided in (FIXME: ZENODO??), and can be run completely with a
...
$x{\le}1000$, and simply provide a reasonable extrapolation to higher $x$ values
...
last 3 terms in the expansions are used as a very conservative estimate on the
...
bounding $f_P(\eta)$ by by simpler functions, it was possible to find a somewhat
looser but still useful bound on the tail contribution in this case as well:
\begin{align}
...
selection of such values in
\reftabrange{yrefvals:P:first}{yrefvals:F:last}. However, it should be noted
that the FIXMEREF also contains a larger number of such values to a much higher
precision in \texttt{JSON} format.
\begin{figure}[tp]
    \centering
    \begin{subfigure}{0.49\textwidth}
        \centering
...
in-continuity at $\cos(2\theta)=0$ in \refeqn{yfitfct:BL}: the coefficients at
first glance looks like they were chosen to avoid this, since
...
mentioned (FIXME: doublecheck), the inaccurate evaluation of \refeqn{yintegral}
to provide reference values of $y_M$. For all models except $M=F$, these
reference values were actually included as tables 1, 3, and 4 in the original
work allowing a direct gauging of their accuracy. These accuracies are shown in
\reftabrange{origtable1precision}{origtable4precision},\footnote{The original
tables were digitised using the \texttt{Tesseract} OCR engine and manually
verified. Any odd entries, such as for $(x,\sin\theta)$=$(4,0.2)$ in
\reftab{origtable1precision} or $(x,\sin\theta)$=$(0.6,0.5)$ in
\reftab{origtable3precision}, were manually double-checked and are likely
a sign of transcription errors in the tables of the original work.  (FIXME: verify once
again? at least any suspicious values).} and clearly there are significant
systematic deviations in particular at higher values of $x$, to some degree
consistent with what is seen in \reffig{prec2dplot}t
...
\begin{figure}[tp]
    ...
    \caption{Precision as per \refeqn{yprecisiondef} of the $y_M(x,\theta)$
      ...
      precision ``luxury'' recipes to be presented in \refsec{newrecipes}.
      Boxes with white and black dashes indicate the limited range of the
      reference values used to tune the recipes in the original work.}
    \labfig{prec2dplot}
\end{figure}
...
\begin{figure}[tp]
    ...
    \end{subfigure}
    \caption{fixme Bla bla cmprecipes. Note: breakdown near 45degrees in $M=L$ plot (weFIXME have split
...
detailed and impressive in the subjective opinion of the authors..

{\tiny
  FIXME:  BC models used a lot, but the issues is by all means not
...
\end{align}
Where $f^-(\eta)$ is only strictly positive when $\eta>1$, so weFIXME will restrict
ourselves to $\eta>2$ (??)

Since $\varphi(\theta,s)$ and $f_P(\eta)$ decreases monotonically for increasing
$s$ and $\eta$ respectively for all positive arguments, weFIXME can conclude that:
\begin{align}
...
\int_{a}^{\infty} f_P^-(\eta)\varphi\left(\theta,xf^+_P(\eta)\right)d\eta \equiv T^-(\theta,x,a)
\end{align}
Using $k=x/a$ and ensuring $a>\max(x,2)$ weFIXME have $k<1$.
...
$2.50$ when compared to the previous order. Thus, if weFIXME restrict ourselves to
$a>10$, weFIXME can conservatively limit the maximal contributions of the terms of
order $1/a^4$ or higher in \refeqn{tplusminus} to be at most $10/a^4$. With that, weFIXME can
...
bound of the error on the estimate. In other words, for $a>\max(10,x)$ weFIXME get:
\begin{align}
\int_{a}^{\infty} f_P(\eta)\varphi\left(\theta,xf_P(\eta)\right)d\eta \approx
...
additional pessimisation of the error bound, weFIXME can arrive at the relatively
...
FIXME BLA BLA \reftabrange{yrefvals:P:first}{yrefvals:F:last}.
...
The contributions of non-authors etc. should be given here. (DMSC cluster?
